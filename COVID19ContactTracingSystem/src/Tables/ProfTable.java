...
public class ProfTable {
    ...
    public ProfTable() throws SQLException, ClassNotFoundException {
        objDBHandler = new DBHandler();
        imgLogo = new ImageModel();
        jFrame = new JFrame("Professor Table");
        ...
        Object[] arrColumn = {"ID", "First name", "Last name","Prof No.",
                "Department","Age" ,"Temperature", "Phone Number",
        ...
        ResultSet objResulset = objDBHandler.profRead();
        ArrayList<Professor> arrProf = new ArrayList<>();
        while (objResulset.next()){
            objProfessor =   new Professor(objResulset.getInt("idprof"), objResulset.getString("firstname"),
                    objResulset.getString("lastname"), objResulset.getString("profno"), objResulset.getString("dept"),
                    objResulset.getInt("age"),objResulset.getDouble("temperature"),objResulset.getString("phonenumber"),
                    objResulset.getString("email"),objResulset.getString("gender"), objResulset.getString("address"),
                    objResulset.getTimestamp("timein"));
            arrProf.add(objProfessor);
        }
        objResulset.close();
        for (Professor objValue : arrProf) {
            arrRow[0] = objValue.getIntProfId();
            arrRow[1] = objValue.getStrFirstName();
            arrRow[2] = objValue.getStrLastName();
            arrRow[3] = objValue.getStrProfNo();
            arrRow[4] = objValue.getStrDepartment();
