...
public class LogisticsTable {
    ...
    Logistics objLogistics;
    protected static DefaultTableModel model;
    public LogisticsTable() throws SQLException, ClassNotFoundException {
        objDBHandler = new DBHandler();
        imgLogo = new ImageModel();
        jFrame = new JFrame("Logistics Table");
        ...
        ResultSet objResultset = objDBHandler.logisticsRead();
        ArrayList<Logistics> arrLogistics = new ArrayList<>();
        while (objResultset.next()){
            objLogistics =   new Logistics(objResultset.getInt("idlogistics"), objResultset.getString("firstname"),
            ...
            arrLogistics.add(objLogistics);
        }
        objResultset.close();
        for (Logistics objValue : arrLogistics) {
            arrRow[0] = objValue.getIntLogisticsID();
            arrRow[1] = objValue.getStrFirstName();
            arrRow[2] = objValue.getStrLastName();
            arrRow[3] = objValue.getStrPlateNo();
            arrRow[4] = objValue.getStrCompany();
                ...
                btnDelete.addActionListener(e1 -> {
                    ...
                    if (i>=0) {
                        try {
                            ...
                            model.removeRow(i);
                            objDBHandler.logisticsDelete(j);
