...
public class StudentTable {
    ...
    public StudentTable() throws SQLException, ClassNotFoundException {
        objDBHandler = new DBHandler();
        imgLogo = new ImageModel();
        jFrame = new JFrame("Student Table");
        ...
        Object[] arrColumn = {"ID", "First name", "Last name","Student No.",
                "Course","Age" ,"Temperature", "Phone Number",
        ...
        ResultSet objResultset = objDBHandler.studentRead();
        ArrayList<Student> arrStudent = new ArrayList<>();
        while (objResultset.next()){
            objStudent =   new Student(objResultset.getInt("idstudent"), objResultset.getString("firstname"),
                    objResultset.getString("lastname"), objResultset.getString("studentno"), objResultset.getString("course"),
            ...
            arrStudent.add(objStudent);
        }
        objResultset.close();
        for (Student value : arrStudent) {
            arrRow[0] = value.getIntStudentID();
            arrRow[1] = value.getStrFirstName();
            arrRow[2] = value.getStrLastName();
            arrRow[3] = value.getStrStudNo();
            arrRow[4] = value.getStrCourse();
            arrRow[5] = value.getIntAge();
            arrRow[6] = value.getDblTemp();
            arrRow[7] = value.getStrPhoneNo();
            arrRow[8] = value.getStrEmail();
            arrRow[9] = value.getStrGender();
            arrRow[10] = value.getStrAddress();
            arrRow[11] = value.getTimeIn();
            model.addRow(arrRow);
        }
