...
public class VendorForm extends JFrame {
    private JLabel lblHeader,lblForm,lblArrow,lblFName, lblLName, lblStallNo,
            lblAge, lblPhoneNo, lblEmail, lblTemp, lblGender,
            lblAddress, lblStallName, lblBg;
    private  JTextField txtFName, txtLName, txtStallNo, txtAge, txtPhoneNo,
            txtEmail, txtTemp, txtGender, txtAddress, txtStallName;
    ...
    public VendorForm() {
        ...
        JComponent[] arrComponents = {lblHeader, lblForm, lblArrow,lblFName,
                                        txtFName, lblLName, txtLName, txtStallNo,
                                        lblStallNo, lblAge, txtAge, lblPhoneNo,
                                        txtPhoneNo, lblEmail, txtEmail, lblStallName,
                                        lblTemp, txtTemp, lblGender, rBtnMale,
                                        rBtnFemale, rBtnNone, rBtnSpecify, txtGender,
                                        lblAddress, txtAddress, btnSubmit, txtStallName};
    ...
    private void components(){
        ...
        lblForm = new JLabel("Vendor Form");
        lblForm.setBounds(204,57,97,25);
        ...
        lblStallNo = new JLabel("Stall No.");
        lblStallNo.setBounds(54,150,70,17);
        txtStallNo = new JTextField();
        txtStallNo.setBounds(54,167,167,25);

        lblStallName = new JLabel("Stall Name");
        lblStallName.setBounds(281,150,150,17);
        txtStallName = new JTextField();
        txtStallName.setBounds(281,167,167,25);
    ...
    private void btnSubmitEvent(){
        btnSubmit.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                objVendor.setStrLastName(txtLName.getText().trim());
                objVendor.setStrFirstName(txtFName.getText().trim());
                objVendor.setStrStallNo(txtStallNo.getText().trim());
                objVendor.setStrStallName(txtStallName.getText().trim());
                objVendor.setStrPhoneNo(txtPhoneNo.getText().trim());
                objVendor.setStrGender(strGender);
                objVendor.setStrAddress(txtAddress.getText().trim());
                objVendor.setTimeIn(timestamp);

                if (!txtEmail.getText().equals(""))
                    strEmail = txtEmail.getText();
                else
                    strEmail = "Not Specified";
                objVendor.setStrEmail(strEmail);

                try {
                    objVendor.setIntAge(Integer.parseInt(txtAge.getText().trim()));
                    objVendor.setDblTemp(Double.parseDouble(txtTemp.getText().trim()));
                    if (!txtFName.getText().equals("") && !txtLName.getText().equals("") && !txtStallNo.getText().equals("") &&
                            !txtAge.getText().equals("") && !txtTemp.getText().equals("") && !txtPhoneNo.getText().equals("") &&
                            !txtStallName.getText().equals("") && !strGender.equals("") && !txtAddress.getText().equals("")) {

                        objDBHandler.vendorWrite(objVendor);
