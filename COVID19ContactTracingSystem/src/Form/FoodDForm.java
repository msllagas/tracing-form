...
public class FoodDForm extends JFrame{
    ...
    private JComboBox<String> cmbFoodDelivery;
    private String strGender = "", strEmail = "";
    private String[] arrFDelivery;
    ...
    public FoodDForm(){
        ...
        arrFDelivery = new String[]{" ---------------------------------", "Food Panda",
                                "Honestbee", "Lalafood Ph", "Grab Food",
                                "Zomato", "Mangan.ph", "Delivery Guy", "Metro Mart"};

        cmbFoodDelivery = new JComboBox<>(arrFDelivery);
        cmbFoodDelivery.setBounds(281,167,167,23);
        components();
        rBtnListener();

        JComponent[] arrComponents = {lblHeader, lblForm, lblArrow,lblFName, cmbFoodDelivery,
    ...
    private void components(){
        ...
        lblForm = new JLabel("Food Delivery Form");
        lblForm.setBounds(183,57,150,25);
    ...
    private void btnSubmitEvent(){
        btnSubmit.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                gender();
                int intIndex = cmbFoodDelivery.getSelectedIndex();
                String strComp = arrFDelivery[intIndex];

                Calendar calendar = Calendar.getInstance();
                Timestamp timestamp = new Timestamp(calendar.getTimeInMillis());
                FoodDelivery objFoodDelivery = new FoodDelivery();
                objFoodDelivery.setStrFirstName(txtFName.getText().trim());
                objFoodDelivery.setStrLastName(txtLName.getText().trim());
                objFoodDelivery.setStrPlateNo(txtPlateNo.getText().trim());
                objFoodDelivery.setStrCompany(strComp);
                objFoodDelivery.setStrPhoneNo(txtPhoneNo.getText().trim());
                objFoodDelivery.setStrGender(strGender);
                objFoodDelivery.setStrAddress(txtAddress.getText().trim());
                objFoodDelivery.setTimeIn(timestamp);

                if (!txtEmail.getText().equals(""))
                    strEmail = txtEmail.getText();
                else
                    strEmail = "Not Specified";
                objFoodDelivery.setStrEmail(strEmail);

                try {
                    objFoodDelivery.setIntAge(Integer.parseInt(txtAge.getText().trim()));
                    objFoodDelivery.setDblTemp(Double.parseDouble(txtTemp.getText().trim()));
                    if (!txtFName.getText().equals("") && !txtLName.getText().equals("") && !txtPlateNo.getText().equals("") &&
                            ...
                            !strComp.equals(arrFDelivery[0]) &&!strGender.equals("") && !txtAddress.getText().equals("")) {

                        objDBHandler.foodDeliveryWrite(objFoodDelivery);
